...
#################### Paths and URLs ####################
# URLs - Format csv
URL_Mairies = "https://www.data.gouv.fr/api/1/datasets/r/d5f400de-ae3f-4966-8cb6-a85c70c6c24a"
URL_Mairies_PLM_Arr= "https://www.data.gouv.fr/api/1/datasets/r/3b6b2281-b9d9-4959-ae9d-c2c166dff118"
...
def calcul_age(date_naissance_str: str, ref_date: datetime) -> int | str:
    ...
    age = (
        ref_date.year
        - d.year
        - ((ref_date.month, ref_date.day) < (d.month, d.day))
    )
...
def reduit_mairies_data(df_raw: pd.DataFrame,  date_elections: datetime | None = None)-> pd.DataFrame | None:
    try:
        ...
        df = df_raw[["Code de la commune", "Code de la collectivité à statut particulier", "Nom de l'élu", "Prénom de l'élu", "Code sexe", "Date de naissance", "Code de la catégorie socio-professionnelle", "Libellé de la fonction", "Code nationalité"]]
        df.rename(columns={
...
def reduit_mairies_plm_arr_data(df_raw: pd.DataFrame,  date_elections: datetime | None = None)-> pd.DataFrame | None:
    try:
        # Garder uniquement les colonnes utiles et les renommer
        ...
        df = df_raw[["Code de la commune", "Libellé du secteur", "Nom de l'élu", "Prénom de l'élu", "Code sexe", "Date de naissance", "Code de la catégorie socio-professionnelle", "Libellé de la fonction"]]
        df.rename(columns={
                        "Code de la commune": "code_commune",
...
def get_mairies_all_data(année: int, output_dir: str, seeds_dir: str, telechargement: bool = False, date_elections: datetime | None = None):
    return get_mairies_data(année,  output_dir, seeds_dir, telechargement, date_elections) \
            and  get_mairies_plm_arr_data(année,  output_dir, seeds_dir, telechargement, date_elections)


def get_mairies_plm_arr_data(année: int, output_dir: str, seeds_dir: str, telechargement: bool = False, date_elections: datetime | None = None):
    try:
        # --- 1. Vérification des dossiers & telechargement raw data---
        output_path = Path(output_dir)
        seeds_path = Path(seeds_dir)

        df_raw = check_dossiers_et_télécharge_raw_data(année, output_path, seeds_path, URL_Mairies_PLM_Arr, MAIRIES_PLM_ARR_RAW_FILENAME, "csv", telechargement)
        ...
        export_and_copy_dbt_source_data(df, année, output_path, seeds_path, MAIRIES_PLM_ARR_FILENAME)
...
def get_mairies_data(année: int, output_dir: str, seeds_dir: str, telechargement: bool = False, date_elections: datetime | None = None):
    try:
        # --- 1. Vérification des dossiers & telechargement raw data---
        output_path = Path(output_dir)
        seeds_path = Path(seeds_dir)

        df_raw = check_dossiers_et_télécharge_raw_data(année, output_path, seeds_path, URL_Mairies, MAIRIES_RAW_FILENAME, "csv", telechargement)
        ...
        export_and_copy_dbt_source_data(df, année, output_path, seeds_path, MAIRIES_FILENAME)
