...
from dbt_run import (
    dbt_run_sources_and_exports,
    get_dbt_models_from_tags,
    dbt_run_oxfam_validated_data,
    init_seeds_oxfam,
    delete_seeds_oxfam,
)
...
from parlement_europeen import get_parlement_europeen_data
from conseils_departements import get_conseils_departements_data
from conseils_regions import get_conseils_regions_data
...
DBT_SEEDS_REF_DIR = DBT_SEEDS_DIR / "referentiels"
DBT_SEEDS_TERRITOIRE_DIR = DBT_SEEDS_DIR / "territoire"
DBT_SEEDS_OXFAM_DIR = DBT_SEEDS_DIR / "oxfam"
DATA_DIR = BASE_DIR / "data"
REF_XL_FILENAME = "IFP_referentiels"
DATA_ADMIN_DIR = DATA_DIR / "administration"
DATA_MAIRIES_DIR = DATA_DIR / "mairies"
DATA_PARLEMENT_EUROPEEN_DIR = DATA_DIR / "parlement_europeen"
DATA_CONSEILS_DEPARTEMENTS_DIR = DATA_DIR / "conseils_departements"
DATA_CONSEILS_REGIONS_DIR = DATA_DIR / "conseils_regions"
DATA_REF_DIR = DATA_DIR / "referentiels"
...
def init_env_dbt() -> str:
    ...
    if not env_path.exists():
        raise FileNotFoundError(
            f"❌ Le fichier d'environnement est introuvable : {env_path}"
        )
    ...
    if not dbt_project_path.exists():
        raise FileNotFoundError(
            f"❌ Le dossier DBT_DIR est introuvable : {dbt_project_path}"
        )
...
def export_data_seeds_xl_to_csv(
    annee: int,
    xl_filename: str,
    data_dir: Path,
    seeds_dir: Path,
    ...
    return excel_to_csv_all_sheets(
        annee, xl_filepath, data_dir, seeds_dir, suffix=suffix
    )
...
# -------------------------
def pipeline_extract_and_export(
    annee: int,
    telechargement: bool = True,
    load_referentiels: bool = True,
    ...
    if load_referentiels:
        ...
        if not ref_xl_filepath.exists():
            raise FileNotFoundError(
                f"❌ Le fichier xl référentiel est introuvable : {ref_xl_filepath}"
            )
        bOK = excel_to_csv_all_sheets(
            annee, ref_xl_filepath, DATA_REF_DIR, DBT_SEEDS_REF_DIR
        )
        if not bOK:
            sys.exit(1)  # échec
    ...
    if load_sources:
        ...
        get_annu_admin_data(
            annee, DATA_ADMIN_DIR, DBT_SEEDS_SOURCES_DIR, telechargement
        )
        # Charge sources mairies
        bOK = get_mairies_all_data(
            annee, DATA_MAIRIES_DIR, DBT_SEEDS_SOURCES_DIR, telechargement
        )
        # Charge sources parlementaires européens
        bOK = get_parlement_europeen_data(
            annee, DATA_PARLEMENT_EUROPEEN_DIR, DBT_SEEDS_SOURCES_DIR, telechargement
        )
        # Charge sources conseils départementaux
        bOK = get_conseils_departements_data(
            annee, DATA_CONSEILS_DEPARTEMENTS_DIR, DBT_SEEDS_SOURCES_DIR, telechargement
        )
        # Charge sources conseils régionaux
        bOK = get_conseils_regions_data(
            annee, DATA_CONSEILS_REGIONS_DIR, DBT_SEEDS_SOURCES_DIR, telechargement
        )
        # sys.exit(0)   # échec
    ...
    if load_territoire:
        ...
        bOK = get_GOG_data(
            annee, DATA_TERRITOIRE_DIR, DBT_SEEDS_TERRITOIRE_DIR, telechargement
        )
    ...
    dbt_run_sources_and_exports(
        str(dbt_project_path), annee, load_referentiels, load_territoire, load_sources
    )
    ...
    if export_models is None:
        logging.info("--- Pas de dbt exports models ---- ")
    else:
        bOK = run_exports(
            annee, export_models, DATA_EXPORTS_DIR, f"administration_exports_{annee}"
        )
        logging.info(
            f"--- Fin des traitements Extract and Export pour l'année {annee} - bOK = {bOK}"
        )
    logging.info(
        "--------------------------------------------------------------------------"
    )
...
def pipeline_import_and_generate(annee: int, db_schema: str, seeds: bool = True):
    ...
    if seeds:
        ...
        bOK = export_data_seeds_xl_to_csv(
            annee, ADM_OXFAM_XL_FILENAME, DATA_OXFAM_DIR, DBT_SEEDS_OXFAM_DIR, "oxfam"
        )
        if not bOK:
            sys.exit(1)  # échec
    ...
    generate_json_executif(
        annee, f"{db_schema}.calc_executif_oxfam", DATA_JSON_DIR / JSON_EXECUTIF_NAME
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--annee", type=int, help="Année à traiter (défaut : année courante)"
    )
    parser.add_argument(
        "--pipeline",
        type=str,
    ...
    parser.add_argument(
        ...
        choices=["y", "n"],
        default="y",
        help="Téléchargement et chargement de tous les fichiers sources, référentiels et territoire inclus (défaut : y)",
    )
    parser.add_argument(
        "--load_referentiels",
        choices=["y", "n"],
        default="y",
        help="Téléchargement des fichiers référentiels (défaut : y)",
    )
    parser.add_argument(
        "--load_territoire",
        choices=["y", "n"],
        default="y",
        help="Téléchargement des fichiers territoire (défaut : y)",
    )
    parser.add_argument(
        "--load_sources",
        choices=["y", "n"],
        default="y",
        help="Téléchargement des fichiers sources (défaut : y)",
    )
    parser.add_argument(
        "--load_oxfam",
        choices=["y", "n"],
        default="y",
        help="Téléchargement des fichiers modifiés par Oxfam (défaut : y)",
    )
    parser.add_argument(
        "--db_schema",
        type=str,
        default="dev",
        help="Schema db des vues finales (défaut : dev)",
    )
...
def main():
    ...
    if pipeline == "extract":
        pipeline_extract_and_export(
            annee, telechargement, load_referentiels, load_territoire, load_sources
        )
        ...
        empty_folder(DATA_PARLEMENT_EUROPEEN_DIR, annee, None)
        empty_folder(DATA_CONSEILS_DEPARTEMENTS_DIR, annee, None)
        empty_folder(DATA_CONSEILS_REGIONS_DIR, annee, None)
