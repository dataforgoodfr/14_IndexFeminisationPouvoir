...
def filtre_et_parse_annu_admin_hierarchie(df_raw: pd.DataFrame) -> pd.DataFrame | None:
    ...
    df = df[
        ["nom", "id", "hierarchie"]
    ]  # df[["nom", "id", "categorie", "type_organisme", "hierarchie"]]
    df = df.rename(columns={"nom": "administration"})
...
def get_annu_admin_data(
    année: int, output_dir: str, seeds_dir: str, telechargement: bool = False
):
    try:
        ...
        export_and_copy_dbt_source_data(
            df, année, output_path, seeds_path, ADMINISTRATION_FILENAME
        )
        ...
        export_and_copy_dbt_source_data(
            df_h, année, output_path, seeds_path, ADMIN_HIER_FILENAME
        )
