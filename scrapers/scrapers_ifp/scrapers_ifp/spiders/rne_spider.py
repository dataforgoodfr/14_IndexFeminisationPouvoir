...
class BaseRneSpider(scrapy.Spider):
    ...
    dataset_api_url: str = 'https://www.data.gouv.fr/api/1/datasets/repertoire-national-des-elus-1/'
    ...
    async def parse_dataset(self, response: TextResponse):
        ...
        if resource_id:
            ...
            self.logger.error(f"❌ [{self.name}] Ressource '{self.resource_filter}' introuvable.")
