...
class Personne(BaseModel):
    ...
    # Type de mandat
    groupe_politique_libelle: Optional[str] = None
    poste_libelle: Optional[str] = None
    zone_geographique_libelle: Optional[str] = None
    zone_geographique_type: Optional[str] = None
    ...
    def validate_genre(cls, v):
        if v not in ALLOWED_GENRES:
            raise ValueError(
                f"Le genre '{v}' n'est pas autorisé. Les valeurs possibles sont : {ALLOWED_GENRES}"
            )
        return v
